...
/**
 * Handler for activating the in the ReviewExplorer selected review. Active when: ReviewExplorer is activePart Enabled when: exactly one
 * MultipleReviewWrapper is selected
 */
                ...
                if (sel1 instanceof IStructuredSelection) {
                    ...
                    if (o instanceof MultipleReviewWrapper) {
                        ...
                        if (!PropertiesManager.getInstance().isReviewOpen(wrap.getReviewId())) {
                            if (!MessageDialog.openConfirm(HandlerUtil.getActiveShell(event), "Activate",
                                    "In order to activate a review, it has to be open. Do you want to open the selected review now?")) {
                                // MessageDialog.openWarning(null, "Warning: Could not activate review", "Only open reviews can be activated");
                                PluginLogger.logWarning("ReviewExplorer", "activateSelectedReview",
                                        "Could not activate review: closed review is selected");
                                return null;
                            }
                            ...
                            try {
                                ...
                                PluginLogger
                                        .logError(
                                                ...
                                                "execute",
                                                "Problems occured executing command \"de.tukl.cs.softech.agilereview.views.reviewexplorer.openCloseSelectedReview\"",
                                                e);
                            } catch (NotDefinedException e) {
                                PluginLogger.logError(this.getClass().toString(), "execute",
                                        "Command \"de.tukl.cs.softech.agilereview.views.reviewexplorer.openCloseSelectedReview\" is not defined", e);
                            } catch (NotEnabledException e) {
                                PluginLogger.logError(this.getClass().toString(), "execute",
                                        "Command \"de.tukl.cs.softech.agilereview.views.reviewexplorer.openCloseSelectedReview\" is not enabled", e);
                            } catch (NotHandledException e) {
                                PluginLogger.logError(this.getClass().toString(), "execute",
                                        "Command \"de.tukl.cs.softech.agilereview.views.reviewexplorer.openCloseSelectedReview\" is not handled", e);
                            }
                        }
