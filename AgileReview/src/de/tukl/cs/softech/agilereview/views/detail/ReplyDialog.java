...
public class ReplyDialog extends Composite implements Listener {
     ...
     */
    private final String strReplyAuthor = PropertiesManager.getPreferences().getString(PropertiesManager.EXTERNAL_KEYS.AUTHOR_NAME);;
    /**
     * inserted reply text
     */
    ...
    private void doOk() {
        ...
        if (strReplyText.equals("")) {
            MessageDialog.openInformation(this.getShell(), "Information", PropertiesManager.getInstance().getInternalProperty(
                    PropertiesManager.INTERNAL_KEYS.COMMENT_EMPTY_REPLY_MESSAGE));
        } else {
            boolSaved = true;
            getParent().dispose();
        }
