...
public class XSLExport implements IRunnableWithProgress {
	...
	/**
	 * Creates a new Instance of XSLExport for a list of reviews, the templatePath where
	 * the export class can find the xls template to be used and the outputPath where the new
	 * instance of the exported template should be stored
	...
	public void run(IProgressMonitor monitor) throws InvocationTargetException,	InterruptedException {
		try {
			...
			MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error while exporting Reviews", "The formulas in the selected template file cannot be evaluated correctly");
		} catch (InvalidFormatException e) {
			PluginLogger.logError(this.getClass().toString(),"run", "InvalidFormatException", e);
			MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error while exporting Reviews", "An error occured while exporting the selected Reviews!");
		} catch (IOException e) {
			PluginLogger.logError(this.getClass().toString(),"run", "IOException", e);
			MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error while exporting Reviews", "One of the selected files could not be read or written!");
		}
	}
	...
	private static ArrayList<FileExportWrapper> getAllWrappedFiles(java.io.File file, String project) {
		...
		final TreeSet<String> omittings = new TreeSet<String>(Arrays.asList(pm.getInternalProperty(PropertiesManager.INTERNAL_KEYS.EXPORT_OMITTINGS).split(",")));
