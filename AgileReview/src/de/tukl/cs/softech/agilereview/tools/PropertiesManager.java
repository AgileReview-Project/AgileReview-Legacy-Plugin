...
/**
 * The PropertiesManager manages the internal configurations (in file:
 * "OSGI-INF/l10n/bundle.properties") as well as the external
 * (workspace-specific) configurations (called preferences)
 */
public class PropertiesManager implements IInputValidator {

	/**
	 * Static class for accessing keys of the internal properties Private keys
	 * should not be accessed directly but via methods specified in
	...
	public static class INTERNAL_KEYS {
		 ...
		 */
		public static String[] DEFAULT_ANNOTATION_COLORS_AUTHOR = new String[] { "annotations.default.color.author0", "annotations.default.color.author1",
				"annotations.default.color.author2", "annotations.default.color.author3", "annotations.default.color.author4",
				"annotations.default.color.author5", "annotations.default.color.author6", "annotations.default.color.author7",
				"annotations.default.color.author8", "annotations.default.color.author9" };
		/**
		...
		/**
		 * Default color of comments, either if colors are disabled or if too
		 * many authors are there
		 */
		public static String DEFAULT_REVIEW_COLOR = "review.default.color";
		/**
		 * Message the display when saving a reply on a comment, without all
		 * fields filled out
		 */
	...
	/**
	 * Static class for accessing keys of the external properties
	 * (=preferences). These preferences are stored in the users workspace and
	 * are therefore workspace-specific. Private keys should not be accessed
	 * directly but via methods specified in {@link PropertiesManager}
	 */
	public static class EXTERNAL_KEYS {
		...
		/**
		 * The source folder where the reviews are stored (path based from
		 * Workspace-root)
		 */
		...
		/**
		 * Indicates whether the last recipient and priority are to be preset
		 * for new comments
		 */
		 ...
		 * Default color of AgileReview annotations for author 1
		 */
		public static String[] ANNOTATION_COLORS_AUTHOR = new String[] { "annotationColorAuthor0", "annotationColorAuthor1", "annotationColorAuthor2",
				"annotationColorAuthor3", "annotationColorAuthor4", "annotationColorAuthor5", "annotationColorAuthor6", "annotationColorAuthor7",
				"annotationColorAuthor8", "annotationColorAuthor9" };
		/**
		...
		/**
		 * States whether the AgileReview Perspective should automatically be
		 * opened on certain events
		 */
		...
		/**
		 * Correlated comment begin tags for every set of file endings
		 * representing the same language
		 */
		public static String PARSER_COMMENT_BEGIN_TAG = "parser_comment_begin_tag";
		/**
		 * Correlated comment end tags for every set of file endings
		 * representing the same language
		 */
		public static String PARSER_COMMENT_END_TAG = "parser_comment_end_tag";
		/**
		 * Indicates whether the user should be asked to create a review folder
		 * if none exists.
		 */
	...
	private PropertiesManager() {
		...
		// FileInputStream("OSGI-INF"+System.getProperty("file.separator")+"l10n"+System.getProperty("file.separator")+"bundle.properties");
		// InputStream stream = new FileInputStream(internalPropertyFile);
		if (stream != null) {
			try {
				this.internalProperties.load(stream);
				stream.close();
				String value = this.internalProperties.getProperty(PropertiesManager.INTERNAL_KEYS.COMMENT_STATUS);
				this.commentStates = value.split(",");
				value = this.internalProperties.getProperty(PropertiesManager.INTERNAL_KEYS.COMMENT_PRIORITIES);
				this.commentPriorities = value.split(",");
			} catch (IOException e) {
	...
	/**
	 * Returns the PreferencesStore to store/retrieve the workspace-specific
	 * preferences of this plugin
	 *
	...
	/**
	 * Adds the given review to the list of open reviews in the
	 * workspace-specific preferences of this plugin
	 *
	 * @param reviewId
	 *            Id of the review to be added
	 */
	...
	/**
	 * Removes the given review from the list of open reviews in the
	 * workspace-specific preferences of this plugin
	 *
	 * @param reviewId
	 *            Id of the review to be removed
	 */
	...
	/**
	 * Returns all reviews which are declared to be "open" in the
	 * workspace-specific preferences of this plugin
	 *
	...
	/**
	 * Checks if the given review is declared as "open" in the
	 * workspace-specific preferences of this plugin
	 *
	 * @param reviewId
	...
	/**
	 * Returns a Comment status value defined in the properties according to its
	 * id
	 *
	 * @param ID
	 *            for the requested Comment status
	...
	/**
	 * Returns a Comment priority value defined in the properties according to
	 * its id
	 *
	 * @param ID
	 *            for the requested Comment priority
	 * @return the String according to the given ID
	 */
	...
	/**
	 * Returns the characters which should not be in all keys (reviewId, author,
	 * commentId)
	 *
	...
	/**
	 * Returns a map of all supported file endings with the correlated comment
	 * tags (first begin, then end tag)
	 *
	 * @return map of all supported file endings with the correlated comment
	 *         tags
	 */
	 ...
	 * @param key
	 *            a static field of
	 *            {@link PropertiesManager.INTERNAL_KEYS.ICONS}
	 * @return an image displaying the icon represented by the key or null if
	 *         there no image could be loaded
	 */
	public Image getIcon(String key) {
		return AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, getInternalProperty(key)).createImage();
	}
}