...
public class AnnotationParser implements IAnnotationParser {
	...
	if (input != null && input instanceof FileEditorInput) {
	    path = ((FileEditorInput) input).getFile().getFullPath().toOSString()
		    .replaceFirst(Pattern.quote(System.getProperty("file.separator")), "");
	}
	...
	try {
	    ...
	    if (startOffset != 0) {
		PluginLogger.logError(this.getClass().toString(), "parseInput",
			"BadLocationException occurs while parsing the editor: " + editor.getTitle(), e);
	    } else {
		// file out of sync or other reasons, so eclipse cannot open file till refresh --> suppress Exception
		PluginLogger.log(this.getClass().toString(), "parseInput",
			"BadLocationException suppressed while parsing the editor: " + editor.getTitle());
	    }
	...
	try {
	    ...
	    PluginLogger.logError(this.getClass().toString(), "parseInput",
		    "CoreException occurs while saving document of editor: " + editor.getTitle(), e);
     ...
     * @param display if true, the new comment will instantly displayed<br>
     *            false, otherwise
