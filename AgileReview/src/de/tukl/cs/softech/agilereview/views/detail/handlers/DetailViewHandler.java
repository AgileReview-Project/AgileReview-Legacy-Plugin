 ...
 */
public class DetailViewHandler extends AbstractHandler {

	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		if(event.getCommand().getId().equals("de.tukl.cs.softech.agilereview.views.detail.revert")) {
			if(ViewControl.isOpen(DetailView.class)) {
				DetailView.getInstance().revert();
			}
		} else
		if(event.getCommand().getId().equals("de.tukl.cs.softech.agilereview.views.detail.newReply")) {
			if(ViewControl.isOpen(DetailView.class)) {
				DetailView.getInstance().addReply();
			}
		} else if(event.getCommand().getId().equals("de.tukl.cs.softech.agilereview.views.detail.delete")) {
			if(ViewControl.isOpen(DetailView.class)) {
