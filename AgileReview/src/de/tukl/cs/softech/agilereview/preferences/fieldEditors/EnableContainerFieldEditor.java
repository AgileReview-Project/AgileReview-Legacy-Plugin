...
/**
 * Advanced Field Editor capable of displaying multiple field editors in a title-bordered container, with a check-box above disabling and enabling the
 * whole container. In order to add field editors to this component, create the field editors using the result of <code>getContainer()</code> as
 * parent and then add the field editor using <code>addField(FieldEditor fieldEditor)</code>. Currently there is no support for validity checking and
 * advanced layout inside the container.
 */
public class EnableContainerFieldEditor extends BorderedFieldEditor {
     ...
     */
    private final String strCheckboxText;
