...
public class RelocateDialog extends Composite implements Listener {
     ...
     */
    private final Comment oldComment;
    
    /**
     * Creates a RelocateDialog on the given parent with a given style. Also the comment which should be moved have to be passed.
                ...
                if ((editor = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor()) != null) {
                    ...
                    if (ctv.openEditorContains(oldComment)) {
                        ...
                        if (input != null && input instanceof FileEditorInput) {
                            ...
                            MessageDialog.openError(this.getShell(), "Comment Detail - Repositioning",
                                    "You cannot relocate this comment as the currently opened editor is not yet supported!");
                        }
                    }
                    
                } else {
                    MessageDialog.openError(this.getShell(), "Comment Detail - Repositioning",
                            "You cannot relocate this comment as there is no editor opened at the moment!");
                }
                
            } catch (IOException e) {
                PluginLogger.log(this.getClass().toString(), "execute",
                        "IOException when trying to create the new moved comment or deleting the old one", e);
            }
            
        } else {
            MessageDialog.openError(Display.getDefault().getActiveShell(), "Comment Detail - Repositioning",
                    "In oder to move a comment, the DetailView and the Comment Summary has to be opened!");
        }
    }
