...
public class AgileCommentFilter extends ViewerFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		...
		if (searchString == null || searchString.length() == 0) {
			...
			if (PropertiesManager.getInstance().getCommentStatusByID(c.getStatus()).matches(searchString) && (restriction.equals("ALL") || restriction.equals("Status"))) {
				matches = true;
			}
			// match in Priority and category ALL or Priority
			if (PropertiesManager.getInstance().getCommentPriorityByID(c.getPriority()).matches(searchString) && (restriction.equals("ALL") || restriction.equals("Priority"))) {
				matches = true;
			}
			...
			if (String.valueOf(c.getReplies().getReplyArray().length).matches(searchString) && (restriction.equals("ALL") || restriction.equals("Replies"))) {
				matches = true;
			}
