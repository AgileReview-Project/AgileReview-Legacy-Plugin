...
public class CommentColorPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
    ...
    protected void createFieldEditors() {
        ColorFieldEditor colorAnnotationField = new ColorFieldEditor(PropertiesManager.EXTERNAL_KEYS.ANNOTATION_COLOR, "Default Comment Color:",
                getFieldEditorParent());
        addField(colorAnnotationField);
        
        EnableContainerFieldEditor containerField = new EnableContainerFieldEditor(PropertiesManager.EXTERNAL_KEYS.ANNOTATION_COLOR_ENABLED,
                "Enable multi-color comments", "Color Settings", getFieldEditorParent());
        // colorfieldeditor for annotation color of IDE user
        AuthorColorFieldEditor authorColorAnnotationField = new AuthorColorFieldEditor(PropertiesManager.EXTERNAL_KEYS.ANNOTATION_COLORS_AUTHOR[0],
                "Comment color (IDE User):", 0, containerField.getContainer());
        ...
        for (int i = 1; i < PropertiesManager.EXTERNAL_KEYS.ANNOTATION_COLORS_AUTHOR.length; i++) {
            authorColorAnnotationField = new AuthorColorFieldEditor(PropertiesManager.EXTERNAL_KEYS.ANNOTATION_COLORS_AUTHOR[i],
                    "Comment color (Author " + (i + 1) + "):", i, containerField.getContainer());
            containerField.addField(authorColorAnnotationField);
        }
