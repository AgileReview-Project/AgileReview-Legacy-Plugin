...
public class CommentTableView extends ViewPart implements IDoubleClickListener {
	 ...
	 */
	private final String[] titles = { "ReviewName", "CommentID", "Author", "Recipient", "Status", "Priority", "Date created", "Date modified", "Replies",
			"Location" };
	/**
	...
	public void addComment(Comment comment) {
		...
		// TODO: Das hier vlt auslagern -> macht CTV dümmer, außerdem liegen
		// z.B. Editor und Selection im Handler vor
		try {
			IEditorPart editor;
			if ((editor = getActiveEditor()) != null) {
				PluginLogger.log(this.getClass().toString(), "addComment", "Add tags for currently added comment");
				this.parserMap.get(editor).addTagsInDocument(comment, getFilteredComments().contains(comment));
			}
	...
	public void deleteComment(Comment comment) {
		...
		try {
			...
			if (openEditorContains(comment)) {
				this.parserMap.get(getActiveEditor()).removeCommentTags(comment);
			} else {
				TagCleaner.removeTag(new Path(ReviewAccess.computePath(comment)), this.ra.generateCommentKey(comment));
				reparseAllEditors();
			}
		} catch (BadLocationException e) {
	...
	public void resetComments() {
		...
		this.comments = this.ra.getAllComments();
		this.viewer.setInput(this.comments);
		this.refreshTable();
	}
	...
	private void filterComments() {
		...
		for (ViewerFilter filter : this.viewer.getFilters()) {
			filteredCommentObjects = filter.filter(this.viewer, this, filteredCommentObjects);
		}
	...
	public void createPartControl(Composite parent) {
		...
		this.comparator = new AgileViewerComparator();
		this.viewer.setComparator(this.comparator);
		this.commentFilter = new AgileCommentFilter("ALL");
		this.viewer.addFilter(this.commentFilter);
	...
	private TableViewer createViewer(Composite parent) {
		...
		this.viewer.addSelectionChangedListener(ViewControl.getInstance());
	...
	private ToolBar createToolBar(final Composite parent) {
		...
		Listener selectionListener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				CommentTableView.this.viewer.removeFilter(CommentTableView.this.commentFilter);
				CommentTableView.this.commentFilter = new AgileCommentFilter(item.getText());
				CommentTableView.this.viewer.addFilter(CommentTableView.this.commentFilter);
		...
		this.filterText.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent ke) {
				CommentTableView.this.commentFilter.setSearchText(CommentTableView.this.filterText.getText());
				CommentTableView.this.viewer.refresh();
				filterComments();
			}

		});
		this.filterText.pack();

		// add seperator to toolbar
		ToolItem itemSeparator = new ToolItem(toolBar, SWT.SEPARATOR);
		itemSeparator.setWidth(this.filterText.getBounds().width);
		itemSeparator.setControl(this.filterText);
		...
		String statusStr = this.pm.getCommentStatusByID(filterStatusNumber);
		...
		onlyOpenCommentsCheckbox.addSelectionListener(new SelectionAdapter() {
			...
			private final ViewerFilter openFilter = new ViewerFilter() {
				...
				public boolean select(Viewer viewer, Object parentElement, Object element) {
					return ((Comment) element).getStatus() == filterStatusNumber; // XXX
					// Hack
				}
			};
			...
			public void widgetSelected(SelectionEvent e) {
				if (onlyOpenCommentsCheckbox.getSelection()) {
					CommentTableView.this.viewer.addFilter(this.openFilter);
				} else {
					CommentTableView.this.viewer.removeFilter(this.openFilter);
				}
				filterComments();
			}
		});
		...
		itemDropDown.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				if (event.detail == SWT.ARROW || event.detail == 0) {
					...
					CommentTableView.this.filterText.setFocus();
				}
			}
		});
	...
	private void createColumns() {
		// ReviewID
		TableViewerColumn col = createColumn(this.titles[0], this.bounds[0], 0);
		...
		// CommentID
		col = createColumn(this.titles[1], this.bounds[1], 1);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
		...
		// Author
		col = createColumn(this.titles[2], this.bounds[2], 2);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
		...
		// Recipient
		col = createColumn(this.titles[3], this.bounds[3], 3);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
		...
		// Status
		col = createColumn(this.titles[4], this.bounds[4], 4);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
				String status = CommentTableView.this.pm.getCommentStatusByID(c.getStatus());
				return status;
			}
		});

		// Priority
		col = createColumn(this.titles[5], this.bounds[5], 5);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
				String prio = CommentTableView.this.pm.getCommentPriorityByID(c.getPriority());
				return prio;
			}
		});

		// Date created
		col = createColumn(this.titles[6], this.bounds[6], 6);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
		...
		// Date modified
		col = createColumn(this.titles[7], this.bounds[7], 7);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
		...
		// Number of relplies
		col = createColumn(this.titles[8], this.bounds[8], 8);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
		...
		// Location
		col = createColumn(this.titles[9], this.bounds[9], 9);
		col.setLabelProvider(new ColumnLabelProvider() {
			@Override
			public String getText(Object element) {
				Comment c = (Comment) element;
	...
	private SelectionAdapter getSelectionAdapter(final TableColumn column, final int index) {
		SelectionAdapter selectionAdapter = new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				CommentTableView.this.comparator.setColumn(index);
				int dir = CommentTableView.this.viewer.getTable().getSortDirection();
				if (CommentTableView.this.viewer.getTable().getSortColumn() == column) {
					dir = dir == SWT.UP ? SWT.DOWN : SWT.UP;
				} else {

					dir = SWT.DOWN;
				}
				CommentTableView.this.viewer.getTable().setSortDirection(dir);
				CommentTableView.this.viewer.getTable().setSortColumn(column);
				CommentTableView.this.viewer.refresh();
			}
		};
	...
	/**
	 * Proves whether the current open editor contains the tags for the given
	 * comment
	 *
	 * @param comment
	 *            the comment
	...
	private HashSet<Comment> getFilteredComments() {
		...
		for (TableItem i : this.viewer.getTable().getItems()) {
	...
	/**
	 * Relocates the comment passed to the current selection within the same
	 * file
	 *
	 * @param comment
	 *            comment which should be relocated
	 */
	public void relocateComment(Comment comment) {
		IEditorPart editor;
		if ((editor = getActiveEditor()) != null) {
			try {
				this.parserMap.get(editor).relocateComment(comment, getFilteredComments().contains(comment));
			} catch (BadLocationException e) {
	...
	/**
	 * Adds the active editor if some is active to the parserMap an creates the
	 * respective AnnotationParser for the normal annotation behavior. <br>
	 * This should be done after refactoring.
	 */
	...
	/**
	 * Triggers the {@link AnnotationParser} of the currently active editor to
	 * reparse its file
	 */
	...
	/**
	 * Triggers all {@link AnnotationParser} in the parser map to reparse its
	 * file
	 */
	...
	/**
	 * Returns all comments which are overlapping with the given
	 * {@link Position}
	 *
	 * @param p
	 *            position
	 * @return all comments which are overlapping with the given
	 *         {@link Position}
	 */
	...
	/**
	 * Computes the next position from the given one on where a comment is
	 * located.
	 *
	 * @param current
	 *            The current position
	 * @return The next position or<br>
	 *         the current position if there is no such position.
	 */
	 ...
	 *
	 * @param event
	 *            will be forwarded from the {@link ViewControl}
	...
	public void selectionChanged(SelectionChangedEvent event) {
		if (event.getSelection() instanceof IStructuredSelection && !event.getSelection().isEmpty()) {
			...
			if (sel.getFirstElement() instanceof AbstractMultipleWrapper) {
				...
				this.viewer.removeFilter(this.selectionFilter);
				...
				if (cmdService != null) {
					...
					if ((Boolean) state) {
						...
						this.viewer.addFilter(this.selectionFilter);
						filterComments();
					}
				}
			}
	...
	public void partInputChanged(IWorkbenchPartReference partRef) {
		if (partRef.getPart(false) instanceof IEditorPart) {
			...
			if (this.parserMap.containsKey(editor) && ViewControl.isPerspectiveOpen()) {
				this.parserMap.get(editor).clearAnnotations();
				this.parserMap.put(editor, ParserFactory.createParser(editor));
				this.parserMap.get(editor).filter(getFilteredComments());
			}
		}
	...
	/**
	 * Removes all annotations if the AgileReview perspective is closed. The
	 * method is invoke by {@link:ViewControl}
	 *
	 * @param page
	 *            the workbench page
	 * @param perspective
	 *            the activated perspective
	 */
	...
	/**
	 * Open correspondent editor of the comment and jump to beginning of that
	 * comment on doubleclick
	 *
	...
	public void selectComment(Comment c) {
		if (getFilteredComments().contains(c)) {
			this.viewer.setSelection(new StructuredSelection(c), true);
		} else {
			if (ViewControl.isOpen(DetailView.class)) {
				DetailView.getInstance().selectionChanged(new SelectionChangedEvent(this.viewer, new StructuredSelection(c)));
			}
		}
	}
		...
		if (this.viewer.getSelection() instanceof IStructuredSelection) {
			...
			if (this.viewer.getSelection().isEmpty()) {
				...
				if (comment instanceof Comment) {
					index = this.comments.indexOf(comment) + 1;
				}
			}
			...
			this.viewer.setSelection(new StructuredSelection(comment));
	...
	private void revealComment(Comment comment) {
		if (openEditor(comment)) {
			...
			try {
				...
				this.parserMap.get(getActiveEditor()).revealCommentLocation(this.ra.generateCommentKey(comment));
			} catch (BadLocationException e) {
			...
			if (ViewControl.isOpen(DetailView.class)) {
				selectComment(comment); // select comment another time to show
				// the comment if the view was closed
				// before
			}
		}
	}
