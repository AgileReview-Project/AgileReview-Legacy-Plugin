...
public class ExceptionHandler {/*?|r108|Malte|c22|?*/
	...
	public static void handleNoReviewSourceFolderException() {
		if(!PropertiesManager.getPreferences().getBoolean(PropertiesManager.EXTERNAL_KEYS.DO_NOT_ASK_FOR_REVIEW_FOLDER)) {/*?|r108|Peter Reuter|c6|*/
			MessageDialogWithToggle md = MessageDialogWithToggle.openYesNoQuestion(Display.getDefault().getActiveShell(), "No ReviewSourceFolder selected",
					...
					"Do not ask me on Eclipse startup.", PropertiesManager.getPreferences().getBoolean(PropertiesManager.EXTERNAL_KEYS.DO_NOT_ASK_FOR_REVIEW_FOLDER), null, null);
	...
	private static void openNewSourceFolderDialog() {
		NoReviewSourceWizard wizard = new NoReviewSourceWizard(false);
